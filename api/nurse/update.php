...
if (!$user || $user->role !== 'admin') {
    // Only admin can update nurses
    http_response_code(403);
    echo json_encode(array("message" => "You don't have permission to update nurses"));
    exit;
}
...
if(!empty($_POST["password"])){
    $nurse->password = password_hash($_POST["password"], PASSWORD_BCRYPT);
}
...
if($nurse->update()){
    ...
    echo json_encode(array(
        "status" => false,
        "message" => "Unable to update nurse."
    ));
}